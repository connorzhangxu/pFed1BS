...
def read_user_data(index, data, dataset):
    ...
    if dataset == "Boston":
        ...
        y_train = torch.Tensor(y_train).type(torch.float32).unsqueeze(1)  # Add extra dimension for regression
        X_test = torch.Tensor(X_test).type(torch.float32)
        y_test = torch.Tensor(y_train).type(torch.float32).unsqueeze(1)
    else:
        X_train = torch.Tensor(X_train).type(torch.float32)
