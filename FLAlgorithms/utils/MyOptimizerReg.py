...
class MyOptimizerReg(torch.optim.Optimizer):
    def __init__(self, params, lr=0.01, alpha=0.1, momentum=0.9, weight_decay=0.0,rho=6e-05,mu=1e-5):
        defaults = dict(lr=lr, momentum=momentum, weight_decay=weight_decay,rho=rho,mu=mu)
        self.alpha = alpha
        super(MyOptimizerReg, self).__init__(params, defaults)
    ...
    def step(self, closure=None, local_net=None, global_net=None, outers_dict=None):
        ...
        with torch.no_grad():
            for group in self.param_groups:
                ...
                sign_outer_product = torch.tanh(outer_product / group['rho'])
                ...
                for param in group['params']:
                    ...
                    grad = (1+group['mu'])*param.grad.data + self.alpha * grad_outer.view(param.shape)
                    # ===================================
    ...
    def step_with_hadamard(self, closure=None, local_net=None, global_net=None, original_shape=None,
        ...
        with torch.no_grad():
            for group in self.param_groups:
                ...
                sign_outer_product = torch.tanh(outer_product / group['rho']).add(-global_net)
                ...
                for i, p in enumerate(params_with_grad):
                    ...
                    full_grad = (1+group['mu'])*p.grad.data + self.alpha * grad_outer.view_as(p.grad.data)
                    # ===================================
                    # lr = group['lr']
